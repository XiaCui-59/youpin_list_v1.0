		...
		<view class="input_btn_wrap"
			...
			<view class="input_box" v-show="!showVoice">
				<input type="text" confirm-type="发送" :value="question" :placeholder="canSend?'更多工作就问我':'请稍后...'"
					...
					:adjust-position="false" style="padding-left: 40rpx;" />
			</view>

			<!-- 右侧输入按钮切换区 -->
			<view class="func_icon" v-show="showSend" @click.stop="sendQuestion">
				<image :src="imgUrl+'/worker/v_list/ic_send.png'" style="border:1px solid #D8E6FF;border-radius: 50%;">
				</image>
			</view>
			<view class="func_icon" @click.stop="changeChatMethod" v-show="!showSend">
				<image :src="showVoice?imgUrl+'/worker/v_list/ic_keyboard.png':imgUrl+'/worker/v_list/ic_voice.png'"
...
<script>
	...
	export default {
		...
		async onLoad(param) {
			...
			if (param.b_id) {
				this.jobFromCast.job_id = param.b_id
				this.jobFromCast.msg = param.b_msg
				this.jobFromCast.type = param.b_type
				this.jobFromCast.action = param.b_action
			}
			this.getSetting()
			if (this.isLogin()) {
				this.creatConnect()
		...
		methods: {
				...
				if (!this.canSend) {
					...
					app.globalData.socketTask.send({
						...
						success(res) {
							_this.jobId = ""
							_this.action = ""
							_this.$set(_this.jobFromCast, "job_id", "")
							_this.$set(_this.jobFromCast, "action", "")
							_this.$set(_this.jobFromCast, "msg", "")
							_this.$set(_this.jobFromCast, "type", "")
			...
			initWebsocket() {
				...
				app.globalData.socketTask.onOpen((res) => {
					...
					if (this.jobFromCast.job_id) {
						this.sendBtnMsg(this.jobFromCast)
					}
				...
				this.prinTimer = setInterval(function() {
					...
					if (index < len) {
						...
						if (_this.responEnd) {
							...
							if (_this.jobFromCast.job_id) {
								_this.sendBtnMsg(_this.jobFromCast)
							}
						}
					}
				}, 30)
